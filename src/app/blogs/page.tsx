...
export const metadata: Metadata = {
  title: "Blog | Computer, Accounts & Marketing Guides",
  description: `Practical guides on Excel, Tally Prime with GST and digital marketing, written by the trainers at ${SITE.name}, ${SITE.address.locality}.`,
    ...
    <div>
      <header className="on-inverse hero-surface relative isolate overflow-hidden px-5 pt-28 pb-14 text-white sm:pt-32 lg:px-8 lg:pb-16">
        ...
        <div className="mx-auto max-w-[1240px]">
          ...
          <Reveal>
            <p className="mt-8 inline-flex items-center rounded-full border border-white/20 bg-white/10 px-4 py-1.5 text-xs font-semibold tracking-wide">Blog</p>
            <h1 className="mt-6 max-w-3xl font-display text-[clamp(2.2rem,5vw,3.6rem)] leading-[1.06] font-extrabold tracking-[-0.03em] text-balance text-white/45">
              Notes from the <span className="text-white">classroom</span> and the <span className="text-accent-yellow">job market.</span>
            </h1>
            <p className="mt-5 max-w-2xl text-base leading-relaxed text-white/75 lg:text-lg">
              Short, practical guides written by our trainers on the skills employers in Punjab actually test for.
            </p>
          </Reveal>
        </div>
      </header>
              ...
              <Reveal as="li" key={post.slug} delay={i * 0.08} className="flex">
                <article className="group relative flex flex-1 flex-col overflow-hidden rounded-card border border-border-subtle bg-surface-raised shadow-card transition-colors hover:border-action/40">
                  ...
                  <div className="flex flex-1 flex-col p-6">
                    <p className="flex items-center gap-2 font-mono text-xs text-content-muted">
                      <time dateTime={post.date}>{formatPostDate(post.date)}</time>
                      <span aria-hidden="true" className="size-1 rounded-full bg-content-muted/50" />
                      {post.readMinutes} min read
                    </p>
                    <h2 className="mt-3 font-display text-lg leading-snug font-bold tracking-tight text-balance">
                      <Link href={`/blogs/${post.slug}`} className="after:absolute after:inset-0 hover:text-action">
                        {post.title}
                      </Link>
                    </h2>
                    <p className="mt-3 flex-1 text-sm leading-relaxed text-content-muted">{post.excerpt}</p>
      ...
      <section className="border-t border-border-subtle bg-surface-sunken px-5 py-16 text-center lg:px-8 lg:py-24">
        <Reveal className="mx-auto max-w-2xl">
          ...
          <p className="mx-auto mt-5 max-w-lg text-base leading-relaxed text-content-muted lg:text-lg">
            Talk to a counsellor. One call is usually enough to know which course fits your goal and your schedule.
          </p>
