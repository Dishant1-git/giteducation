...
import { certificateHref, certificatePrograms, TEL_HREF } from "@/lib/site";

export default function CertificateProgramNotFound() {
  return (
    <div className="hero-surface on-inverse relative isolate overflow-hidden px-5 pt-32 pb-20 text-white">
      ...
      <div className="mx-auto max-w-3xl text-center">
        ...
        <h1 className="mt-4 font-display text-3xl leading-tight font-extrabold tracking-tight text-balance sm:text-4xl">We could not find that certificate program</h1>
        <p className="mx-auto mt-4 max-w-xl text-base leading-relaxed text-white/70">
          The link may be out of date, or the program may have been renamed. Every certificate program we currently run is listed below.
        </p>
        ...
        <ul className="mx-auto mt-10 grid max-w-2xl gap-2 text-left sm:grid-cols-2">
          {certificatePrograms.map((program) => (
            <li key={program.slug}>
              ...
              >
                <Icon name={program.icon} className="size-4 shrink-0 text-accent-yellow" />
                {program.label}
              </Link>
            </li>
          ))}
        ...
        <div className="mt-10 flex flex-col justify-center gap-3 sm:flex-row">
          <Link href="/certificate-programs" className="inline-flex h-12 items-center justify-center rounded-full bg-accent-yellow px-7 font-semibold text-ink transition-colors hover:bg-white">
